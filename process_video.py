...
def calc_time(frame_num, ifps):
    return ifps * frame_num

def proc_vid(vid_path, ifps, show_frames):
    global red_on
    ...
    while(vid.isOpened()):
        ...
        if area_red > upper_contour_bound:
            t = calc_time(frame_cnt, ifps)
            setRed(True, control_list, t)
        elif (area_red < lower_contour_bound or len(contours_red) == 0) and red_on:
            t = calc_time(frame_cnt, ifps)
            setRed(False, control_list, t)
...
# args: video_path, fps, clist_output_name
if __name__ == '__main__':
    file_name = "./control_lists/" + sys.argv[3] + ".clist"
    vid_path = sys.argv[1]
    fps = int(sys.argv[2])
    b = sys.argv[4]
    show_frames = True
    if b.lower() == "false":
        show_frames = False
    control_list = proc_vid(vid_path, 1/fps, show_frames)
